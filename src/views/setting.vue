<template>
    <div class="awards" style="width:900px;margin:0 auto">
        ...
        <Card shadow>
            ...
            <div slot="extra">
                <Button type="info" @click.prevent.native="updateAwardFun('add')">添加</Button>
            </div>
            <div class="l-table">
                <Table :columns="awardData.columns" :data="awardData.data"></Table>
            </div>
        </Card>
        ...
        <Card shadow>
            ...
            <div slot="extra">
                <Button type="info" @click.prevent.native="updateMemberFun('add')">添加</Button>
            </div>
            <div class="l-table">
                <Table :columns="joinMemberData.columns" :data="joinMemberData.showdata"></Table>
            </div>
            <div class="pageToolbar">
                <Page 
                    :total="joinMemberData.total" show-elevator show-sizer show-total placement="top" 
                    :current.sync="joinMemberData.offset" :page-size-opts="joinMemberData.pageSize" :page-size="joinMemberData.limit" 
                    @on-change="(val)=>{this.pagingChange(val,joinMemberData)}" 
                    @on-page-size-change="(val)=>{this.pagesizeChange(val,joinMemberData)}"></Page>
            </div>
        </Card>

        <!--奖励 添加编辑模态框start -->
        <Modal v-model="awardData.popMadol.show" :mask-closable="false" width="800" height='500' @on-cancel="closeHandle('formValidate')">
            <p slot="header">
                <Icon type="md-information-circle" style="color:#19be6b;"></Icon>
                <span>{{`${awardData.popMadol.type === 'add' ? '添加': '编辑'}`}}</span>
            </p>
            <div class="modal-content">
                <Form ref="formValidate" :model="awardData.popMadol.data" :rules="formValidate" label-position="right" :label-width="120">
                    <FormItem label="名称：" prop="name">
                        <Input placeholder="请输入名称" type="text" v-model="awardData.popMadol.data.name"></Input>
                    </FormItem>
                    <FormItem label="数量：" prop="count">
                        <Input placeholder="请输入数量" type="text" v-model="awardData.popMadol.data.count"></Input>
                    </FormItem>
                    <FormItem label="奖品：" prop="award">
                        <Input placeholder="请输入奖品" type="text" v-model="awardData.popMadol.data.award"></Input>
                    </FormItem>
                </Form>
            </div>
            <div slot="footer">
        ...
        <!--奖励 添加编辑模态框end -->
         ...
         <!--参与人员 添加编辑模态框start -->
        <Modal v-model="joinMemberData.popMadol.show" :mask-closable="false" width="800" height='500' @on-cancel="closeHandle('joinMemberDataDOM')">
            <p slot="header">
                <Icon type="md-information-circle" style="color:#19be6b;"></Icon>
                <span>{{`${joinMemberData.popMadol.type === 'add' ? '添加': '编辑'}`}}</span>
            </p>
            <div class="modal-content">
                <Form ref="joinMemberDataDOM" :model="joinMemberData.popMadol.data" :rules="joinMemberDataDOM" label-position="right" :label-width="120">
                    <FormItem label="姓名：" prop="name">
                        <Input placeholder="请输入姓名" type="text" v-model="joinMemberData.popMadol.data.name"></Input>
                    </FormItem>
                    <FormItem label="工号：" prop="count">
                        <Input placeholder="请输入数量" type="text" v-model="joinMemberData.popMadol.data.id"></Input>
                    </FormItem>
                </Form>
            </div>
            <div slot="footer">
                <Button type="success" @click="saveHandle('joinMemberDataDOM')">确定</Button>
                <Button type="info" @click="closeHandle('joinMemberDataDOM')">取消</Button>
            </div>
        </Modal>  
        <!--参与人员 添加编辑模态框end -->
    </div>   
...
export default {
    ...
    // 初始化操作类
        ...
        initTableColumn(){
            ...
            let awardDataColumns = [], joinMemberDataColumns=[];
                    ...
                    render: (h, params) => {
                        return h('div',[ 
                            h( "Button",{
                                ...
                                on: { click: () => { this.updateAwardFun('edit',params.row); } }
                            }, "编辑" ),
                            h( "Button",{
                                props: { type: "error", size: "small" },
                                style: { marginRight: "5px"  },
                                on: { click: () => { this.deleteAwardFun(params.row); } }
                            }, "删除" ),
                        ])
                    }
                }
                    ...
                    render: (h, params) => {
                        return h('div',[ 
                            h( "Button",{
                                props: { type: "primary", size: "small" },
                                style: { marginRight: "5px"  },
                                on: { click: () => { this.updateMemberFun('edit',params.row); } }
                            }, "编辑" ),
                            h( "Button",{
                                props: { type: "error", size: "small" },
                                style: { marginRight: "5px"  },
                                on: { click: () => { this.deleteMemberFun(params.row); } }
                            }, "删除" ),
                        ])
                    }
                }
        ...
        initTableData(){
            ...
            let prizeData = [], memberData = [] ;
            
            localstorage.fetch('menber').length ? memberData = localstorage.fetch('menber') : memberData = getMemberData;
            localstorage.fetch('prize').length  ? prizeData = localstorage.fetch('prize') : prizeData = getPrizeData;
                 ...
                 onOk: () => {
                    setTimeout(() => {
                        ...
                        awardData.forEach((item,index)=>{
                            if(`${item['name']}${item['award']}` === `${row['name']}${row['award']}`){
...
<style>
.pageToolbar { text-align: right; margin-top: 10px;}
</style>
