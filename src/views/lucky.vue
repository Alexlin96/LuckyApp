...
export default {
  ...
  methods: {
    ...
    initDataFn() {
      ...
      let awards = localstorage.fetch("prize").length
      ...
      let result = localstorage.fetch("result").length
    ...
    otherInit() {
      ...
      let members = localstorage.fetch("menber").length
        ? localstorage.fetch("menber")
        : [];
      this.$set(this.baseData, "members", members);

      let players = localstorage.fetch("players").length
    ...
    onReplay() {
      this.$Modal.confirm({
        ...
        onOk: () => {
          setTimeout(() => {
            this.baseData.batchPlayers.splice(0);
            let p = this.baseData.result.splice(
